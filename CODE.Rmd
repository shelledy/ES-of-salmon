...
hFWE <- ggplot() + 
  geom_bar(data=ES[complete.cases(ES[, "EE_domain"]), ], aes(fct_inorder(EE_domain), fill = Category), alpha = 0.7, color = "black", width = 0.5) + theme_cowplot() + scale_y_continuous(expand = expansion(mult = c(0, 0.05)), breaks = seq(0, 150, by=50)) + theme(axis.title.x=element_blank(), legend.title=element_blank()) + scale_fill_manual(values = c("#332288", "#CC6677","#44AA99")) +
